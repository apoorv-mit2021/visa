...
import {Layers, CheckCircle, Package, Gauge} from "lucide-react";
import Badge from "../../ui/badge/Badge";
import {getCollectionMetrics} from "../../../services/collectionService";
import type {CollectionMetrics as CollectionMetricsType} from "../../../services/collectionService";
import axios from "axios";

export default function CollectionMetrics({refreshKey = 0}: { refreshKey?: number }) {
    const [metrics, setMetrics] = useState<CollectionMetricsType | null>(null);
    ...
    useEffect(() => {
        const fetchMetrics = async () => {
            try {
                setLoading(true);
                const data = await getCollectionMetrics(token);
                setMetrics(data);
            } catch (error: unknown) {
                ...
                if (axios.isAxiosError(error)) {
                    const message =
                        ...
                        "Failed to load collection metrics.";
